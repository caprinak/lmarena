...
export function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'marketing' | 'product' | 'production' | 'finance' | 'reports'>('dashboard');
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-6">
            ...
            <StatHeader label="Market Share" value={`${latestStats.marketShare.toFixed(1)}%`} />
            ...
            >
              ...
              {activeTab === 'marketing' && (
                <MarketingDecisions 
                  ...
                  onChange={(m: Decisions['marketing']) => updateDecisions({ marketing: m })} 
                />
              )}
              {activeTab === 'product' && (
                <ProductDecisions 
