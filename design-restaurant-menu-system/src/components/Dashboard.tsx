...
export const Dashboard: React.FC<DashboardProps> = ({ gameState }) => {
  ...
  const chartData = gameState.history.map(turn => ({
    year: `Y${turn.year}`,
    revenue: turn.stats.revenue / 1000000,
  ...
  const revenueTrend = getTrend(latestStats.revenue, previousStats.revenue);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          label="Net Revenue" 
          value={`$${(latestStats.revenue / 1000000).toFixed(1)}M`} 
          trend={revenueTrend} 
          subtext="Total company sales"
        />
        <StatCard 
          label="Net Profit" 
        ...
        <StatCard 
          ...
          icon={DollarSign} 
          subtext="Profit per share"
        />
        <StatCard 
          label="Image Rating" 
          value={latestStats.imageRating.toString()} 
          icon={Star} 
          ...
          <div className="bg-indigo-900 text-white p-8 rounded-3xl shadow-xl relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="space-y-4">
                ...
                <HealthMetric label="Market Share" value={`${latestStats.marketShare.toFixed(1)}%`} progress={latestStats.marketShare * 3} />
              </div>
            </div>
